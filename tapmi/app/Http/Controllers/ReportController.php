...
class ReportController extends Controller
{
	...
	public function download_proses(Request $req)
	{
		...
		// Report EBCC Validation
		else if (Input::get('REPORT_TYPE') == 'EBCC_VALIDATION_ESTATE') {
			if (Input::get('REGION_CODE') != '' && Input::get('COMP_CODE') == '') {
			...
			self::download_excel_ebcc_validation($setup, 'estate');
		} else if (Input::get('REPORT_TYPE') == 'EBCC_VALIDATION_MILL') {
			if (Input::get('REGION_CODE') != '' && Input::get('COMP_CODE') == '') {
				$setup['WERKS_AFD_BLOCK_CODE'] = substr(Input::get('REGION_CODE'), 1, 1);
			} else if (Input::get('COMP_CODE') != '' && Input::get('BA_CODE') == '') {
				$setup['WERKS_AFD_BLOCK_CODE'] = Input::get('COMP_CODE');
			...
			self::download_excel_ebcc_validation($setup, 'mill');
		}
		// Report Class Block
	...
	public function download_excel_ebcc_validation($data, $param)
	{
		...
		$ebcc_validation = Data::web_report_ebcc_validation_find('/' . $data['WERKS_AFD_BLOCK_CODE'] . '/' . $data['START_DATE'] . '/' . $data['END_DATE'] . '/' . $param);
	...
	public function excel_inspeksi($data, $output = 'excel')
	{
		...
		if (isset($inspection_header)) :
			...
			array_multisort(
				array_column($inspection_header, 'BA_CODE'),
				SORT_ASC,
				array_column($inspection_header, 'AFD_CODE'),
				SORT_ASC,
				array_column($inspection_header, 'BLOCK_NAME'),
				SORT_ASC,
				array_column($inspection_header, 'INSPECTION_DATE'),
				SORT_ASC,
				$inspection_header
			);
